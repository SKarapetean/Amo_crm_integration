...
{
    logMessage('Contact create request:', 'hook.txt', $requestData);

    $data = [];
    if (isset($requestData['contacts']['add'])) {
        $requestData = $requestData['contacts']['add'];
    ...
    logMessage('Contact edit success:', 'hook.txt', $out);
}
...
function contactEdit(array $requestData = []): void
{
    ...
    if (isset($requestData['contacts']['edit'])) {
        $requestData = $requestData['contacts']['edit'];
        if ($requestData['type'] === 'contact') {
            $data['id'] = $requestData['id'];
            $textData = 'Name: ' . $requestData['name']
                . ' Responsible user id: ' . $requestData['responsible_user_id']
...
function sendContactEditRequest(array $data, string $accessToken,  ?int $contactId = null): array
{
    $uri = 'api/v4/contacts/';
    if ($contactId) {
        $uri .= '/' . $contactId;
